...
internal static class FileTools
{
		...
		bool made = false;
			...
			{ 
				...
				{ 
					...
					error = $"Can't find drive for {path}";
			}
			catch (Exception exception)
			{ 
				error = $"Error where make path {path}. {exception.Message}";
			}
	...
	internal static string LoadTextFile(string fileName, System.Text.Encoding encoding)
	{
		System.Text.StringBuilder content = new();
			...
			// Carga el archivo
			using (StreamReader file = new(fileName, encoding))
			{ 
				string data;
	...
	{
		using (StreamWriter file = new(fileName, false, encoding))
		{ 
			file.Write(text);
		}
	}
